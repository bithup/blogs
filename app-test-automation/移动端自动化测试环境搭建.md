...
- 安装JDK、Python、Nodejs、Appium-Desktop对应的macOS版版
- 安装XCode 10，如果已安装XCode升级到10.x以上版本
- 获取apple开发者账号及证书，收费与免费的均可

### 配置环境

- macOS有三个配置文件可修改环境变量：/etc/profile > /etc/paths > ~/.bash_profile；前两个是系统级的，后面是用户级别的
- 配置Java环境

```bash
# 切换到用户目录
cd ~
# vim编辑.bash_profile，用图像界面编辑open -e .bash_profile
vim .bash_profile
# 将下面的配置添加到文件末尾，JAVA_HOME后面的路径需要修改为实际安装目录
...
```bash
# macOS自带python2，安装python3后，直接使用python命令，默认执行的是
# python2
# 同样编辑 .bash_profile，加入如下配置，其中的python安装路径需要修改为真实路径
...
## <span id="caps">Appium 服务器初始化参数</span>
